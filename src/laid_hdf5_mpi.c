...
int main(int argc, char** argv)
{
	...
	if (!skip_dm_creation)
	{
		...
		else
		{
			...
			dm.s_offset			   = BLOCK_LOW(rank, size, n_matrix_lines);
			dm.s_size			   = BLOCK_SIZE(rank, size, n_matrix_lines);
		}
		...
		mpi_hdf5_open_dataset(args.filename, DM_LINE_DATA, comm, MPI_INFO_NULL,
							  &hdf5_dset);

		uint32_t n_lines = roundUp(dm.n_matrix_lines, WORD_BITS);
		word_t* buffer	 = (word_t*) calloc(n_lines, sizeof(word_t));
		word_t* buffer2	 = (word_t*) calloc(n_lines, sizeof(word_t));
		...
		mpi_read_one_column(&hdf5_dset, &dm, rank, buffer);
		MPI_Barrier(comm);
		...
		mpi_calculate_one_column(&dataset, &dm, rank, buffer2);
		MPI_Barrier(comm);
